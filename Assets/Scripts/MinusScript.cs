...
public class MinusScript : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
        ...
        {
            if ((child.localPosition - transform.parent.transform.localPosition).magnitude < (0.5 * transform.parent.transform.parent.localScale.x) && child != transform.parent.transform)
            {
                transform.parent.transform.localPosition = rowScript.GetOriginalPos();
                bool worked = child.GetComponent<RowScript>().Subtraction(rowScript.getNumbers());
                if (worked)
                {
                    soundFX.playMouseUpSubtractionClip();
                }
            }
        }
        transform.parent.transform.localPosition = rowScript.GetOriginalPos();
